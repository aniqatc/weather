...
class WeatherService {
  constructor(apiKey) {
    ...
    this.forecastData = 'https://api.openweathermap.org/data/3.0/onecall';
    this.units = 'imperial';
  ...
  async byName(location) {
    return await axios.get(`${this.generalData}?q=${location}&appid=${this.apiKey}&units=${this.units}`);
  }
  ...
  async fetchDailyForecast(coordinates) {
    try {
      ...
      console.error('Unable to load weekly forecast', error);
      dailyWeather.forecastContainer.innerHTML =
        '<p class="forecast-error">Weekly forecast unavailable. Try again later.</p>';
    }
  }
  ...
  displaySelectedLocationWeather(location) {
    this.byName(location).then((response) => selectedLocationWeather.displayCurrentTemperature(response));
  }
...
const selectedLocationWeather = {
  ...
  displayCurrentTemperature: function (response) {
    if (response.status == 200) {
      ...
      weatherService.renderIcons(document, data.weather[0].id, data.weather[0].icon, `.default-main-icon`);
  ...
  displayForecast: function (response) {
    ...
    forecastData.forEach((day, index) => {
      if (index < 7) {
        ...
        weatherService.renderIcons(this.forecastContainer, day.weather[0].id, day.weather[0].icon, `#icon-${index}`);
      }
    });
  },
};
...
const globalWeather = {
  ...
  getGlobalTemperatures: function () {
    this.countryRows.forEach((item, i) => {
      weatherService.byName(this.randomCities[i]).then((response) => this.displayGlobalTemperatures(response, i, item));
    });
  },
  ...
  displayGlobalTemperatures: function (response, i, item) {
    ...
    weatherService.renderIcons(item, response.data.weather[0].id, response.data.weather[0].icon, '.global-icon');
  },
